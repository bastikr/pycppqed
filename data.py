...
class StateVector(BlitzArray):
    ...
    Usage
    -----
    >> ba = StateVector("(0,1)x(0,1)", "[ (1,2) (3,4) \n  (5,6) (7,8) ]", 0.2)
    ...
    time
        A number giving the point of time when this StateVector was reached.
    """
...
class Trajectory:
    ...
    def plot(self, show=True):
        ...
        show
            ...
            a window containing the plot will appear automatically. (Default
            is True)
        """
        import pylab
...
class CppqedOutputReader:
    ...
    read
        ...
        Otherwise it is necessary to call the "read" method separately. This
        only needs to be done explicitly if you want access the attributes
        "datastr" or "commentstr" directly. 
    """
    ...
    def saveascii(self, path, update=False, traj=True, svs=True):
        ...
        svs
            If set True all the State Vectors are written to different files
            named like "path_time.sv". (Default is True)
        """
        if update or self.datastr is None:
            self.read()
