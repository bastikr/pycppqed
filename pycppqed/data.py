...
class StateVector(BlitzArray):
    ...
    **Arguments**
        ...
        * *time*
            A number giving the point of time when this StateVector was
            reached.
    """
...
class Info:
    ...
    **Arguments**
        * *commentstr*
            A string following the style of the comment section of C++QED output
            files.
    """
...
class Trajectory:
    ...
    **Arguments**
        * *traj*
            ...
            tuples for different timesteps. The next inner tuples hold tuples
...
class CppqedOutputReader:
    ...
    **Arguments**
    ...
    def _parsedatastr(self, traj_handler, sv_handler):
        """
        Call the appropriate handler for each StateVector and Trajectory entry.
        """
        buf = self.datastr
        pos = 0
        while True:
    ...
    def savemat(self, path, update=True, traj=True, svs=True, split=False):
        ...
        **Arguments**
            * *path*
                The path where the output should be written. For StateVectors
